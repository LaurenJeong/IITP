		...
		nexacro._createSysEvent_ForwardFuncs = function (_cur_win)
		{
			...
			_cur_win._syshandler_onfocusin_forward = function (evt) // using focus in
			{
				if (nexacro._enableaccessibility && nexacro._accessibilitytype == 2)
				{
					...
					if (win && elem)
					{
						var refer_comp = win.findComponent(elem);
						if (refer_comp)
						{
							...
							if (comp)
							{
								...
								comp._accessibility_focusin(refer_comp, related_comp);
							}
						}
					}
...
*/
	...
	if (nexacro._Browser != "IE" || (nexacro._Browser == "IE" && nexacro._OSVersion >= 6.0))// NT6.0 : Vista
	{
	    nexacro._syshandler_onkeydown = function (_sysEvent, node, evt)
		{
	        ...
	        if (win && elem)
			{
				...
				if (nexacro._enableaccessibility && nexacro._accessibilitytype == 2)
				{
					win._virtual_cursor_keydown = false; // init
					var accesibility_keyaction = (keycode == nexacro.Event.KEY_TAB || keycode == nexacro.Event.KEY_SPACE ||
						(keycode >= nexacro.Event.KEY_LEFT && keycode <= nexacro.Event.KEY_DOWN))
					if (accesibility_keyaction)
					{
						win._virtual_cursor_keydown = true;
					}
				}
	...
	{		
		...
		if (win && elem)
		{
			...
			if (nexacro._enableaccessibility && nexacro._accessibilitytype == 2)
			{
				// keydown 없이 발생한 탭, 상하 방향키, 스페이스에 대한 처리 필요시 사용
				var accesibility_keyaction = (keycode == nexacro.Event.KEY_TAB || keycode == nexacro.Event.KEY_SPACE
					|| (keycode >= nexacro.Event.KEY_LEFT && keycode <= nexacro.Event.KEY_DOWN));
				if (accesibility_keyaction && win._virtual_cursor_keydown == false)
				{					
					...
					{	
						up_comp._accessibility_keyaction(up_elem, refer_comp, keycode, evt.altKey, evt.ctrlKey, evt.shiftKey, evt.metaKey);
					}
				}
				win._virtual_cursor_keydown = false;
			}
    ...
    {   
    	...
    	switch (type)
		{
			...
			case 2: localstoragekey = localstoragekey + "engine"; break;
