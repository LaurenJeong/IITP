    ...
    nexacro._errortable =
        {
            ...
            "zh":
            {
                ...
                "syntax_fail_parse_xml": ["句法分析时出错。[", "%0", "]"],
                "syntax_not_set_samepath": ["无法设置为与form相同的路径。[", "%0", "]"],
                "loading_error": ["%0", " 缓冲时发生error。 "],
                ...
                "native_notimplemented": ["无法实现该功能。[", "%0", "]"],
                "native_notsupport": ["HTML5不支持该对象或属性 。", "%0", "[", "%1", "]"],
                "native_deprecate": ["该功能是Deprecate功能。 [", "%0", "]"],

                "native_fail_dataset_head": ["加载数据集头信息时失败 。"],
                "native_fail_dataset_recode": ["加载数据集记录信息时失败 。"],
                "native_exist_id": ["已存在使用该ID的Object。id = [", "%0", "]"],
                "native_exist_parent": ["parent已存在。 "],
                "native_notexist_parent": ["parent不存在。"],
                "native_notexist_licensefile": ["授权文件不存在。"],
                "native_expire_license": ["授权时间[", "%0", "]已到期。"],
                "native_invalid_licensefile": ["授权信息无效。请确认授权。"],
                "native_invalid_licensedate": ["授权时间无效。有效期限 [", "%0", "]", "-[", "%1", "]"],
                "native_invalid_licenseactivation": ["授权激活(activation)日期无效。当前日期(基准日期)[", "%0", "]不得早于激活日期[", "%1", "]。"],
                "native_license_invaliddomain": ["授权域名信息不正确。需要确认域名。"],
                "native_license_fail_parse": ["授权域名信息不正确。分析授权文件时出错。 [", "%0", "]"],
                "native_fail_openfile": ["无法打开文件。[", "%0", "]"],
                "native_license_invalidservice": ["不支持该服务的授权。请确认授权。"],
                ...
                "native_check_loadmodule": ["内部加载的模块必须包含证书，但是找不到。确定要允许加载吗？"],
                ...
                "comm_stop_transaction_byesc": ["Esc Key使Transaction停止(非标准。) "],
                ...
                "msg_access_invalid_object": ["该对象会诱发任意命令执行漏洞。\n确定要继续吗？\n"],
                "msg_permit_object": ["总是允许该对象"],

                "msg_update_processing": ["有正在运行的Process[", "%0", "]，Update失败。如果想自动退出正在运行的Process，请选择Yes(是)；如果想直接退出正在运行的Process后继续运行， 请选择 No(否)；如果想在Restart后Update，请选择Cancel(取消) 。"],
                "msg_reset_sandboxdialog": ["随着程序的改善，必须在文件夹/域名访问警告窗口中重新设置现已设置的是否允许。请顾客多多谅解。"],

                "msg_start": ["启动Nexacro。"],
                "msg_search_proxyserver": ["正在搜索代理服务器配置。"],
                "msg_update_engine": ["更新引擎。"],
                "msg_download": ["下载ADL内容。"],

                "comp_incorrect_file": ["指定的文件不正确。 "],
                "comp_fail_loadactivex": ["执行ActiveX/Applet时出错。 [", "%0", "]"],

                "object_push_send_byec": ["Push服务器发送“Byec”或按照与服务器的约定结束。"],
                "object_push_socket_timeout": ["已暂停。"],
                "object_push_fail_command_auth": ["Command Auth Error。当前所选IP的认证出错。"],
                "object_push_fail_all_command_auth": ["Iplist属性中注册的Push服务器的连接全部失败。"],
                "object_push_fail_data_auth": ["数据传输信道认证(authentication)出错。"],
                "object_push_fail_connect": ["服务器连接失败。"],
                "object_push_fail_send_receive": ["数据包传输(send/recv)出错。"],
                "object_push_connected_already": ["服务器连接成功。"],
                "object_push_fail_disconnect": ["解除与X-PUSH服务器连接(disconnect)的请求失败。"],
                "object_push_fail_suspend": ["X-PUSH服务器发出的停止接收消息(suspend)的请求失败 。"],
                "object_push_fail_resume": ["X-PUSH服务器发出的重新开始接收消信(resume)的请求失败 。"],
                "object_push_fail_command": ["发送至X-PUSH服务器的topic注册/注销请求失败。"],
                "object_push_notfound_layouturl": ["找不到消件布局定义文件。"],
                "object_push_invalid_layouturl": ["消件布局定义文件无效。"],
                "object_push_fail_protocol_version": ["消件格式版本信息不一致。"],
                "object_push_fail_rect": ["接收、响应、发送可靠消息失败。"],
                "object_push_fail_reqd": ["未接收消息请求失败。"],
                ...
                "msg_accessibility_emptyline": ["空列"],
