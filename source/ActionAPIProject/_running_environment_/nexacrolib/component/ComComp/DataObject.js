...
if (!nexacro.DataObject)
{
    ...
    */
    ...
    nexacro.DataObjectDataChangedEventInfo = function (obj, id, url, reason, datalist, dsid)//, progressload)
    {
    ...
    nexacro.DataObjectValueChangedEventInfo = function (obj, id, url, reason, path, index, key, newvalue, dsid)//, progressload)
    {
    ...
    _pDataObject._event_list =
        {
            "onsuccess": 1, "onload": 1, "onerror": 1, "ondatachanged" : 1,"onvaluechanged" : 1
        };
    ...
    */
    ...
    _pDataObject.on_apply_enableevent = function (v)
    {
        if (v)
        {
            ...
            this.on_fire_ondatachanged(-1, -1, 41, this.getUpdatelist()); //nexacro.Dataset.REASON_ENABLEEVENT
            //Todo..
            /*
    ...
    */
    ...
    _pDataObject.setDataValue = function (path, index, key, value, srcdsid)
    {
        //path 문법
        //$.body.server[*]
        // /body/servers/
        if (index == -1)
        {
            ...
            this.on_fire_onvaluechanged(0, "SUCCESS", 16, /*rowobj,*/ "", index, key, value, srcdsid); //test code
        }
        else
        {
            ...
            if (rootobj)
            {
                ...
                {
                    rowobj[key] = value;
                    this._updatelist.push({ "uid": "update" + parseInt(Math.random() * 100000000), "type": "update", "index": index, "data": rowobj, "dataobjectpath": path, "srcdatasetid": srcdsid });
                    if (this.enableevent)
                    {
                        this._before_fire_event = true;
                        this.on_fire_onvaluechanged(0, "SUCCESS", 1, /*rowobj,*/ path, index, key, value, srcdsid); //test code
    ...
    _pDataObject.setData = function (path, type, index, data, srcdsid)
    {
        ...
        this._updatelist.push({ "uid": type + parseInt(Math.random() * 100000000), "type": type, "index": index, "data": data, "dataobjectpath": path, "srcdatasetid": srcdsid });
        if (this.enableevent && rowobj)
        {
            this._before_fire_event = true;
            this.on_fire_ondatachanged(0, "SUCCESS", 1, this._updatelist, srcdsid); //test code
            this._before_fire_event = false;
        }
    };
    ...
    _pDataObject.setDataList = function (path, datalist, srcdsid)
    {
        ...
        for (var i = 0, len = datalist.length; i < len; i++)
        {
            ...
            if (rootobj)
            {
                ...
                //if (this.updatecontrol)
                updatalist.push({ "uid": type + parseInt(Math.random() * 100000000), "type": type, "index": index, "data": obj, "dataobjectpath": path, "srcdatasetid": srcdsid });
                if (this.enableevent)
                    bfire = true;
            }            
    ...
    _pDataObject.getUpdatelist = function ()
    {
    ...
    _pDataObject.clearUpdatelist = function ()
    {
        this._updatelist.length = 0;
        this._remaining_updatelist.length = 0;
    };

    _pDataObject.removeUpdateitem = function (uid)
    {
    ...
    _pDataObject.on_fire_ondatachanged = function (errcode, errmsg, reason, datalist, dsid)
    {
        if (errcode >= 0)
        {
            // based on manual
        ...
        if (event && event._has_handlers)
        {
            var evt = new nexacro.DataObjectDataChangedEventInfo(this, "ondatachanged", "", /*errcode, errmsg,*/ reason, datalist, dsid);
            event._fireEvent(this, evt);
        }
    ...
    _pDataObject.on_fire_onvaluechanged = function (errcode, errmsg, reason, datapath, index, key, oldvalue, newvalue, dsid)
    {
        if (errcode >= 0)
        {
            // based on manual
        ...
        if (event && event._has_handlers)
        {
            var evt = new nexacro.DataObjectValueChangedEventInfo(this, "onvaluechanged", "", /*errcode, errmsg,*/ reason, datapath, index, key, oldvalue, newvalue, dsid);
            event._fireEvent(this, evt);
        }
    ...
    */
    ...
    _pDataObject._onDataObjectRequestError = function (obj, bfireevent, errorobj, errortype, url, returncode, requesturi, locationuri, extramsg, svcid, response, response_headers)
    {
        ...
        if (bfireevent)
        {
            ...
            if (environment)
            {
                environment.on_fire_onerror(obj, /*commerrorobj.name*/"ObjectError", commerrorobj.message, errorobj, returncode, requesturi, locationuri, svcid);
            }
