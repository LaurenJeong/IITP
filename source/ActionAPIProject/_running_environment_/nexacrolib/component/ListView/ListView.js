    ...
    */
	...
	_pListView._getItemViewCountRow = function (rowfirst)
	{
		...
		if (expandtype == "expand" || expandtype == "accordion")
		{
			...
			if (itemsize <= 0) return rc > 0 ? rc : 1;
		...
		if (expandtype == "expand" || expandtype == "accordion")
		{
			if (this._use_scrollmanager && this._scrollmanager)
			{
				...
				else
				{
					...
					var item = this._getItem(start);
					if (item)
					{
						// row에 배치된 아이템중 최대 높이
						if (nexacro._isArray(item))
						{
							for (i = start; i < start + col; i += item.length)
							{
								item_maxheight_row = Math.max(item_maxheight_row, this._getItemHeight(panel_idx, -9));
								panel_idx++;
							}
						}
						else
