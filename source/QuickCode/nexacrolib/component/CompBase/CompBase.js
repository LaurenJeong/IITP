...
if (!nexacro.Component)
{
    ...
    _pComponent.createComponent = function (bCreateOnly)
    {
        ...
        {
            if (!bCreateOnly && this.parent._control_element && this.parent._control_element.handle)
            {
    ...
    {
        var _parent_elem = (parent_elem.parent_elem) ? parent_elem.parent_elem : this._getForm()._control_element; // genie : control element 공유할 때 parent elem이 없는 경우 접근 필요
        var control_elem = new nexacro.ControlElement(_parent_elem);
        control_elem.setLinkedControl(this);
        this._control_element = control_elem;

        //this.parent._control_element.parent = this;
        //this.parent._control_element.linkedcontrol = this;

        // genie : destroy 시 삭제되는 property가 있는데 AC와 ctrlobj가 같은 객체를 참조하면 안되는걸까?
        this.parent._control_element = control_elem; 
    ...
    {
        var _parent_elem = parent_elem;
        if (this._is_control_component)
            _parent_elem = parent_elem.parent_elem;
        var control_elem = new nexacro.ScrollableControlElement(_parent_elem, this._use_container_multi);
        control_elem.setLinkedControl(this);
        if (this._is_control_component)
            this.parent._control_element = control_elem;
        this._control_element = control_elem;
    ...
    _pComponent._applyElementVisible = function (v) // HTML visibility vs display
    {
        ...
        {
            if (this._isFluidContainer() && !this._isFluidContainerCalculation())
