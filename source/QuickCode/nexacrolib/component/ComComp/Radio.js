...
if (!nexacro.Radio)
{
    ...
    _pRadio._on_radio_onkeydown = function (obj, evt) //keycode, alt_key, ctrl_key, shift_key, refer_comp)
    {
        var E = nexacro.Event;
        ...
        else if (keycode == E.KEY_LEFT || keycode == E.KEY_RIGHT || keycode == E.KEY_UP || keycode == E.KEY_DOWN) // radioitem 이동
        {
            ...
            if (row_cnt > 0)
            {
                ...
                var op = 0;
                if (keycode == E.KEY_LEFT || keycode == E.KEY_UP)
                {
                    op = -1;
                }
                else if (keycode == E.KEY_RIGHT || keycode == E.KEY_DOWN)
                {
                    op = 1;
                }
                ...
                if (ret)
                {
                    ...
                    if (nexacro._enableaccessibility)
                    {
                        var evt_name = undefined;
                        if (op == -1)
                            evt_name = "keyleft";
                        else if (op == 1)
                            evt_name = "keyright";

                        radioitem._on_focus(true, evt_name);
                        radioitem._changeUserStatus("selected", true);
                    }
                }
            }
