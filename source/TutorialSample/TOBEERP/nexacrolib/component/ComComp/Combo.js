...
if (!nexacro.Combo)
{
    ...
    /*
    _pCombo.on_apply_accessibility = function (accessibility)
    {
        nexacro.Component.prototype.on_apply_accessibility.call(this, accessibility);
        if (this.comboedit)
        {
            this.comboedit.on_apply_accessibility(accessibility);
        }
    };
    */
    ...
    _pCombo._showPopup = function (ds, index, use_timer)
    {
        ...
        if (!nexacro._isDesktop() && !(nexacro._Browser == "Runtime" && nexacro._OS == "Android") && use_timer && !nexacro.test)
        {
            var thisP = this;
		...
		if (this.on_fire_ondropdown(this))
        {
            ...
            if (combolist)
			{
                ...
                if (nexacro._enableaccessibility)
                {
                    ...
                    if (ds && comboedit && comboedit._input_element)
                    {
						...
						if (item)
						{
							item._setAccessibilityActiveDescendant(input_elem);
						}
        ...
        if (comboedit)
        {
            ...
            if (text != v)
            {
                ...
                if (nexacro._enableaccessibility)
                {
                    ...
                    {
                        // 팝업시 선택되는 item을 출력
                        if (this.combolist && this._isPopupVisible())
                        {                            
                            var ds = this._selectDataset();
                            if (ds)
                            {
                                var idx = this._getIndexFromText(ds, v);
                                var item = this.combolist._getItem(idx);
                                if (item)
                                {
                                    item._setAccessibilityActiveDescendant(comboedit._input_element);
                                }
                            }
                        }
